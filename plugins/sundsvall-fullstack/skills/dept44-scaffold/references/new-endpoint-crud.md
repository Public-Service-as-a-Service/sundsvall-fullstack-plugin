...
- `{Name}ResourceTest.java` — happy path, `@SpringBootTest`, `WebTestClient`, `@MockitoBean`
- `{Name}ResourceFailureTest.java` — validation failures, use URL-safe invalid values (e.g. `"bad-municipality-id"`, `"not-a-valid-uuid"`) — do NOT use `#` in constants since it is a URL fragment delimiter
